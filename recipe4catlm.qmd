...
```{python}
last_plot().layer_data(i=1).head()  # the fitted y (not the raw data y) is of interest
```
:::
...
Below we use the new function `geom_cat_lm()`, contrasting it to `geom_smooth()`, which produce parallel and non-parallel slopes, respectively.
...
```{pyodide}
#| exercise: geom_cat_fitted_0
#| envir: geom_cat_fitted
#| autorun: true
#| edit: false
import pandas as pd
import statsmodels.formula.api as smf
...
```{pyodide}
#| exercise: geom_cat_fitted_1
#| envir: geom_cat_fitted
# Define
def compute_panel_cat_fitted(data, scales=None):
    model = smf.ols("y ~ x + C(cat)", data=___).fit()
    return data.assign(y=model.fittedvalues)

# Define
def compute_panel_cat_residuals(data, scales=None):
    model = smf.ols("y ~ x + C(cat)", data=data).fit()
    return ___.assign(xend=data["x"], yend=model.fittedvalues)


# Test compute_panel_cat_fitted
(
    penguins_clean
    .rename(columns={"bill_depth_mm": "x", "bill_length_mm": "___", "species": "cat"})
    [["x", "y", "cat"]]
...
```{pyodide}
#| exercise: geom_cat_fitted_2
#| envir: geom_cat_fitted
# Define
class stat_cat_fitted(stat):
    REQUIRED_AES = {"x", "___", "cat"}
...
(
    ...
    + geom_segment(stat=___, color="maroon")
    + geom_point()
)
```
...
```{pyodide}
#| exercise: geom_cat_fitted_3
#| envir: geom_cat_fitted
# Define
class geom_cat_fitted(___):
    DEFAULT_PARAMS = {"stat": "cat_fitted"}

# Define
class geom_cat_residuals(___):
